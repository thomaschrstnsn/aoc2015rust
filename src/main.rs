use advent_of_code::template::commands::{all, download, read, scaffold, solve, scaffold_file};
...
fn main() {
    match parse() {
        ...
        Ok(args) => match args {
            ...
            AppArguments::Scaffold { day, file } => if file { scaffold_file::handle(day) } else { scaffold::handle(day)}
            AppArguments::Solve {
                day,
                release,
                time,
